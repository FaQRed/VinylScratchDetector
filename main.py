...
crnn_model = tf.keras.models.load_model('models/crnn_model/vinyl_crnn_model.keras')
...
def clean_results(scratches):
    if not scratches:
        return {"status": "clean", "total": 0, "seconds": []}
    filtered = []
    last_s = -10
    for s in sorted(scratches, key=lambda x: x["second"]):
        if s["second"] - last_s > 1.5:
            filtered.append(s)
            last_s = s["second"]
    return {"status": "scratch_detected", "total": len(filtered), "seconds": filtered}
...
@app.post("/analyze/crnn")
async def analyze_crnn(file: UploadFile = File(...)):
    sr = 16000
    y = await load_audio_from_upload(file, sr)
    found = []
    step, win = int(HOP_SEC * sr), int(WINDOW_SEC * sr)
